...
const loginCaptain = async (req, res) => {
    try {
        ...
        responseFormat(res, error.code || ResponseCode.INTERNAL_SERVER_ERROR, error.message)
    }
}
...
const getProfile = async (req,res) => {
    try {
        return responseFormat(res,ResponseCode?.OK,"Profile fetch successfully",req.captain)
    } catch (error) {
        responseFormat(res,error?.code || ResponseCode?.INTERNAL_SERVER_ERROR,error?.message)
    }
}
...
module.exports = { loginCaptain, signUpCaptain, getProfile,updateProfile };